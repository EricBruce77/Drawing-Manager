-- Migration: Update search_drawings function to include project search
-- This allows searching by customer name, project name, and project number
...
BEGIN
    ...
    SELECT
        ...
        (
            ...
            CASE WHEN LOWER(COALESCE(p.name, '')) LIKE '%' || LOWER(search_query) || '%' THEN 25.0
            ELSE 0.0 END
            +
            -- Project number match gets good score (20)
            CASE WHEN LOWER(COALESCE(p.project_number, '')) LIKE '%' || LOWER(search_query) || '%' THEN 20.0
            ELSE 0.0 END
            +
    ...
    WHERE
        ...
        (
            ...
            -- OR match project name
            OR LOWER(COALESCE(p.name, '')) LIKE '%' || LOWER(search_query) || '%'
            -- OR match project number
            OR LOWER(COALESCE(p.project_number, '')) LIKE '%' || LOWER(search_query) || '%'
